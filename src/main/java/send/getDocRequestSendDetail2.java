...
@WebServlet(name = "docrequestsend-detail2", urlPatterns = {"/getDocRequestSendDetail2"})
public class getDocRequestSendDetail2 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!request.getParameter("wh").equals("")) {
                wh_code = request.getParameter("wh");
            }
            JSONArray jsarrDetail = new JSONArray();
            String query2 = "select *,COALESCE((select price_" + price + " from ic_inventory_price_formula where ic_code = ic_transfer_detail_temp.item_code and unit_code = ic_transfer_detail_temp.unit_code limit 1),'0')as price,COALESCE((select name_1 from ic_unit where ic_unit.code = ic_transfer_detail_temp.unit_code),'')as un_name,\n"
                    ...
                    + "                              where ic_trans_detail.wh_code = '" + wh_code + "' and ic_trans_detail.last_status=0 and ic_trans_detail.item_type<>5 and (select item_type from ic_inventory where ic_inventory.code = ic_trans_detail.item_code)<>1 and doc_date_calc<='now()' group by item_code,wh_code ) as temp1 \n"
                    + "                              on temp1.ic_code = ic_inventory.code where temp1.ic_code = ic_transfer_detail_temp.item_code  and ic_unit_code = ic_transfer_detail_temp.unit_code "
                    + "                            ) as temp2 ),0)as average_cost_end\n"
            ...
            while (__rsHead2.next()) {
                ...
                obj2.put("average_cost", __rsHead2.getString("average_cost_end"));

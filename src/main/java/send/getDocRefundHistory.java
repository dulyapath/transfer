...
@WebServlet(name = "refunddochistory-sale", urlPatterns = {"/getRequestDocRefundHistory"})
public class getDocRefundHistory extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!request.getParameter("fd").equals("")) {
            from_date = " and rim_date between '" + request.getParameter("fd") + "' and '" + request.getParameter("td") + "' ";
        }
        ...
        try {
            ...
            //String query1 = "select * from (select *,(select doc_date from ic_trans where doc_no=wid_doc) as wid_date,to_char((select doc_date from ic_trans where doc_no=wid_doc),'DD/MM/YYYY') as wid_datex,coalesce((select doc_time::text from ic_trans where doc_no=wid_doc),'') as wid_time,to_char(doc_date,'DD/MM/YYYY') as doc_datex,to_char(doc_time,'HH24:MI') as doc_timex,COALESCE((select name_1 from ic_warehouse where ic_warehouse.code = wh_code),'')as wh_name,COALESCE((select name_1 from ic_warehouse where ic_warehouse.code = to_wh_code),'')as to_wh_name,COALESCE((select name_1 from ic_shelf where ic_shelf.code = shelf_code and ic_shelf.whcode = wh_code),'')as shelf_name,COALESCE((select name_1 from ic_shelf where ic_shelf.code = to_shelf_code and ic_shelf.whcode = to_wh_code),'')as to_sh_name,COALESCE((select name_1 from erp_branch_list where erp_branch_list.code = branch_code),'')as branch_name,COALESCE((select name_1 from erp_branch_list where erp_branch_list.code = to_branch_code),'')as to_branch_name,COALESCE((select name_1 from erp_user where erp_user.code = user_code),'')as user_name from ic_transfer_doc_temp where branch_code in (" + _bhList + ") and wh_code in (" + _whList + ") and shelf_code in (" + _shList + ") and status in (2,3,4) and wid_doc != '' order by create_datetime desc) as temp where 1=1 " + from_date + search + " limit 50";
            String query1 = " select * from \n"
                    + " (select *,(select doc_date from ic_trans where doc_no=rim_doc) as rim_date,(select creator_code from ic_trans where doc_no=rim_doc) as rim_user,(select remark from ic_trans where doc_no=rim_doc) as rim_remark,to_char((select doc_date from ic_trans where doc_no=rim_doc),'DD/MM/YYYY') as rim_datex,\n"
                    + " coalesce((select doc_time::text from ic_trans where doc_no=rim_doc),'') as rim_time,to_char(doc_date,'DD/MM/YYYY') as doc_datex,to_char(doc_time,'HH24:MI') as doc_timex,\n"
                    + " COALESCE((select name_1 from ic_warehouse where ic_warehouse.code = wh_code),'')as wh_name,COALESCE((select name_1 from ic_warehouse where ic_warehouse.code = to_wh_code),'')as to_wh_name,\n"
                    + " COALESCE((select name_1 from ic_shelf where ic_shelf.code = shelf_code and ic_shelf.whcode = wh_code),'')as shelf_name,COALESCE((select name_1 from ic_shelf where ic_shelf.code = to_shelf_code and ic_shelf.whcode = to_wh_code),\n"
                    + " '')as to_sh_name,COALESCE((select name_1 from erp_branch_list where erp_branch_list.code = branch_code),'')as branch_name,COALESCE((select name_1 from erp_branch_list where erp_branch_list.code = to_branch_code),'')\n"
                    + " as to_branch_name,COALESCE((select name_1 from erp_user where erp_user.code = user_code),'')as user_name from ic_transfer_doc_temp \n"
                    + " where branch_code in (" + _bhList + ") and wh_code in (" + _whList + ") and shelf_code in (" + _shList + ") and status in (2,3,4) and rim_doc != '' and rim_doc in (select doc_no from ic_trans where doc_no like '%MRIM%') order by create_datetime desc)\n"
                    + "  as temp where 1=1  " + from_date + search + " limit 100";
            ...
            while (__rsHead.next()) {
                ...
                obj.put("doc_date", __rsHead.getString("rim_datex"));
                obj.put("doc_time", __rsHead.getString("rim_time"));

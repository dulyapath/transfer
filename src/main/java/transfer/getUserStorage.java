...
@WebServlet(name = "userstorage-save", urlPatterns = {"/getUserStorage"})
public class getUserStorage extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String query1 = "select *  from erp_user_storage where emp_code = '" + __user + "'";
            ...
            while (__rsHead.next()) {
                ...
                _sess.setAttribute("to_branch_code", __rsHead.getString("to_branch_code"));
                _sess.setAttribute("to_wh_code", __rsHead.getString("to_wh_code"));
                _sess.setAttribute("to_shelf_code", __rsHead.getString("to_shelf_code"));
                _sess.setAttribute("rev_branch_code", __rsHead.getString("rev_branch_code"));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (row > 0) {
                PreparedStatement __stmt_update = __conn.prepareStatement("update erp_user_storage set branch_code='" + __branch_code + "',wh_code='" + __wh_code + "',shelf_code='" + __sh_code + "' ,to_branch_code='" + __to_branch_code + "',to_wh_code = '" + __to_wh_code + "' ,to_shelf_code = '" + __to_sh_code + "' where emp_code = '" + __emp_code + "';");
                __stmt_update.executeUpdate();
                __stmt_update.close();
            } else {
                _insert_trans.append("insert into erp_user_storage (emp_code,branch_code,wh_code,shelf_code,to_branch_code,to_wh_code,to_shelf_code) "
                        + "values ('" + __emp_code + "','" + __branch_code + "','" + __wh_code + "','" + __sh_code + "','" + __to_branch_code + "','" + __to_wh_code + "','" + __to_sh_code + "');");

...
@WebServlet(name = "approvedirect-save", urlPatterns = {"/approveDirect"})
public class approveDirect extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (row > 0) {
                ...
                _insert_trans_sale_details_temp.append("insert into ic_trans_detail (trans_type,trans_flag,doc_date,doc_time,doc_no,item_code,item_name,unit_code,line_number,qty,branch_code,wh_code,shelf_code,average_cost,sum_of_cost,stand_value,divide_value,ratio,calc_flag)  "
                        + " select trans_type,trans_flag,doc_date,doc_time,doc_no,item_code,item_name,unit_code,line_number,qty,branch_code,wh_code,shelf_code,average_cost,sum_of_cost,stand_value,divide_value,ratio,calc_flag from ic_transfer_trans_detail_temp where doc_no = '" + __doc_no + "'");
                ...
                PreparedStatement __stmt_updatex = __conn.prepareStatement("update ic_transfer_doc_temp  set status = 2 where doc_no = '" + __doc_no + "';");
                __stmt_updatex.executeUpdate();
                __stmt_updatex.close();

...
@WebServlet(name = "docrefund-detail", urlPatterns = {"/getDocRefundDetail"})
public class getDocRefundDetail extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String query1 = "select *,to_char(doc_date,'DD/MM/YYYY') as doc_datex,doc_time as doc_timex,COALESCE((select name_1 from ic_warehouse where ic_warehouse.code = wh_from),'')as wh_name,COALESCE((select name_1 from ic_shelf where ic_shelf.code = location_from and ic_shelf.whcode = wh_from),'')as shelf_name,COALESCE((select name_1 from erp_branch_list where erp_branch_list.code = branch_code),'')as branch_name,COALESCE((select name_1 from erp_user where erp_user.code = creator_code),'')as user_name from ic_transfer_trans_temp where doc_no='" + request.getParameter("docno") + "'";
            ...
            String query2 = "select *,COALESCE((select price_0 from ic_inventory_price_formula where ic_code = ic_transfer_trans_detail_temp.item_code and unit_code = ic_transfer_trans_detail_temp.unit_code limit 1),'')as price_0,COALESCE((select name_1 from ic_unit where ic_unit.code = ic_transfer_trans_detail_temp.unit_code),'')as un_name from ic_transfer_trans_detail_temp where doc_no = '" + request.getParameter("docno") + "' order by line_number";

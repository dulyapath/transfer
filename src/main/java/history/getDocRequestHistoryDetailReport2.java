...
@WebServlet(name = "docrequesthistoryreport2-detail", urlPatterns = {"/getDocRequestHistoryDetailReport2"})
public class getDocRequestHistoryDetailReport2 extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String query2 = "select *,COALESCE((select price_0 from ic_inventory_price_formula where ic_code = ic_transfer_detail_temp.item_code and unit_code = ic_transfer_detail_temp.unit_code limit 1),'')as price,COALESCE((select name_1 from ic_unit where ic_unit.code = ic_transfer_detail_temp.unit_code),'')as un_name,\n"
                    ...
                    + "                              temp1.branch_code,temp1.wh_code \n"
                    + "                              from ic_inventory \n"
                    + "                              left join (select \n"
                    ...
                    + "                              ,branch_code,wh_code from ic_trans_detail \n"
                    + "                              where ic_trans_detail.branch_code = '" + branch_code + "' and ic_trans_detail.wh_code = '" + wh_code + "' and ic_trans_detail.last_status=0 and ic_trans_detail.item_type<>5 and (select item_type from ic_inventory where ic_inventory.code = ic_trans_detail.item_code)<>1 and doc_date_calc<='now()' group by item_code,branch_code,wh_code ) as temp1 \n"
                    ...
                    + "                            from ic_transfer_detail_temp where doc_no = '" + request.getParameter("docno") + "' and receive_qty > 0 order by line_number";

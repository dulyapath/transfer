...
@WebServlet(name = "docrequestrec2-save", urlPatterns = {"/saveDocRequestReceive2"})
public class saveDocRequestReceive2 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            _insert_trans_sale_temp.append("insert into ic_transfer_doc_temp (wid_doc,status,doc_no,doc_date,remark,user_code,wh_code,shelf_code,branch_code,to_wh_code,to_shelf_code,to_branch_code) "
                    + "values ('" + __wid_doc + "',2,'" + __doc_no + "','" + __doc_date + "','" + __remark + "','" + __user_code + "','" + __from_wh + "','" + __from_sh + "','" + __from_bh + "','" + __to_wh + "','" + __to_sh + "','" + __to_bh + "')");
            System.out.println(_insert_trans_sale_temp.toString());
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject obj = jsonArray.getJSONObject(i);
                _insert_trans_sale_details_temp.append("insert into ic_transfer_detail_temp (line_number,doc_no,item_code,item_name,unit_code,qty,balance,event_qty,average_cost,receive_qty) values "
                        + "(" + obj.get("line_number") + ",'" + __doc_no + "','" + obj.get("item_code") + "','" + obj.get("item_name") + "','" + obj.get("unit_code") + "'," + obj.get("qty") + "," + obj.get("balance") + "," + obj.get("event_qty") + "," + obj.get("average_cost") + "," + obj.get("receive_qty") + ");");
            }

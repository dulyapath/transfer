...
@WebServlet(name = "saverimtemp-save", urlPatterns = {"/saveDocRimTemp"})
public class saveDocRimTemp extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String __doc_time = request.getParameter("doc_time");
        ...
        String __rt_doc = request.getParameter("rt_doc");
        ...
        try {
            ...
            String query1 = "select doc_no  from ic_transfer_doc_temp where doc_no = '" + __rt_doc + "'";
            System.out.println("query1 " + query1);
            PreparedStatement __stmt = __conn.prepareStatement(query1, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            ResultSet __rsHead = __stmt.executeQuery();
            __rsHead.next();
            ...
            if (row > 0) {
                PreparedStatement __stmt_delete = __conn.prepareStatement("update ic_transfer_doc_temp set status=4 , rim_doc = '" + __doc_no + "' where doc_no = '" + __rt_doc + "';");
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                _insert_trans_sale_details_temp.append("insert into ic_transfer_trans_detail_temp (ref_doc_no,trans_type,trans_flag,doc_date,doc_time,doc_no,item_code,item_name,unit_code,line_number,qty,branch_code,wh_code,shelf_code,price,sum_amount,stand_value,divide_value,ratio,calc_flag) values "
                        + "('" + __wid_doc + "',3,58,'" + __doc_date + "','" + __doc_time + "','" + __doc_no + "','" + obj.get("item_code") + "','" + obj.get("item_name") + "','" + obj.get("unit_code") + "','" + obj.get("line_number") + "','" + obj.get("qty") + "','" + __from_bh + "','" + __from_wh + "','" + __from_sh + "','" + obj.get("average_cost") + "','" + obj.get("sum_of_cost") + "',"
            ...
            _insert_trans_sale_temp.append("insert into ic_transfer_trans_temp (trans_type,trans_flag,doc_date,doc_no,doc_time,branch_code,wh_from,location_from,doc_format_code,creator_code,last_editor_code,remark,total_amount) "
                    + "values (3,58,'" + __doc_date + "','" + __doc_no + "','" + __doc_time + "','" + __from_bh + "','" + __from_wh + "','" + __from_sh + "','MRIM','" + __user + "','" + __user + "','สร้างจาก "+__rt_doc+"'," + sum_amount + ");");
            _insert_trans_sale_temp.append("insert into ap_ar_trans_detail_temp (trans_flag,trans_type,doc_date,doc_no,billing_no,billing_date) "
                    + "values(58,3,'" + __doc_date + "','" + __doc_no + "','" + __wid_doc + "','" + __doc_date + "')");

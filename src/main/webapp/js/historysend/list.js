...
function _getListData(data) {
    ...
    $.ajax({
        url: serverURL + 'getRequestDocSendHistory?fd=' + from_date + '&td=' + to_date + '&search=' + data,
        method: 'GET',
        cache: false,
        success: function (res) {
...
function _showListDetail(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += `    <div class="card-body">
                <div class="row">
                    <div class="col-12">
                        <div class="text-header text-left">${data[i].wid_doc} <span class="text-header text-right" style="color:${color}">${statusText}</span></div>
                    </div>
        ...
        if (data[i].doc_no != '') {
            html += `   <div class="col-4">
                        <div class="text-body text-left">เลขที่ใบขอ : ${data[i].doc_no}</div>
                    </div>`
        }
        ...
        if (data[i].status == "1") {
            ...
            html += ` <button  class="btn btn-danger btn-icon-split cancel_doc" data-docno="${data[i].doc_no}" data-status="${data[i].status}">
                            <span class="icon text-white-50">

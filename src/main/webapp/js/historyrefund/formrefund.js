...
$(document).ready(function () {
    ...
    var datetime = 'MRT' + currentdate.getFullYear() + '' + (currentdate.getMonth() + 1) + '' + currentdate.getDate() + '' + currentdate.getHours() + '' + currentdate.getMinutes() + '' + currentdate.getSeconds() + '' + uuidv4().toUpperCase()
...
function _getDocDetail(docno) {
    $('.loading').show();
    $.ajax({
        url: serverURL + 'getDocRefundHistoryDetail?docno=' + docno,
        method: 'GET',
        cache: false,
        success: function (res) {
            console.log(res);
            if (res.length > 0) {
                $('#doc_no').val(res[0].rim_doc);
                $('#doc_no').attr("readonly", "true");
                $('#doc_date').val(res[0].rim_date);
                $('#remark').val(res[0].rim_remark);

...
function _getDocDetail(docno) {
    $('.loading').show();
    $.ajax({
        url: serverURL + 'getShowDocSendDetail?docno=' + docno,
        method: 'GET',
        cache: false,
        success: function (res) {
            console.log(res);
            if (res.length > 0) {
                ...
                $('#price_formular').val('ราคา ' + res[0].price_formula);
...
function _displayTable() {
    ...
    for (var i = 0; i < item_detail.length; i++) {
        ...
        html += '<td class="text-center" >' + formatNumber(parseFloat(item_detail[i].price)) + '</td>'

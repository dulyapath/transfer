...
$(document).ready(function () {
    ...
    $('#wid_docno').val(datetime);
    $('#wid_docdate').val(currentdate.getFullYear() + '-' + ('0' + (currentdate.getMonth() + 1)).slice(-2) + '-' + ('0' + currentdate.getDate()).slice(-2));
    $('#wid_remark').val('');
                        ...
                        for (var i = 0; i < item_detail.length; i++) {
                            ...
                            if (parseFloat(item_detail[i].event_qty) < parseFloat(item_detail[i].qty)) {
                                ...
                                var json_data = {
                                    ...
                                    balance_qty: 0,
                                    event_qty: parseFloat(item_detail[i].qty) - parseFloat(item_detail[i].event_qty),
                                    average_cost: 0,
                                    sum_of_cost: 0
                                }
...
function _getDocDetail(docno) {
    $('.loading').show();
    var formula = $('#price_formula').val();
    $.ajax({
        url: serverURL + 'getDocRequestSendDetail?docno=' + docno + '&p=' + formula,
        method: 'GET',
        cache: false,
        success: function (res) {
            console.log(res);
...
function _getDocDetail2() {
    ...
    var formula = $('#price_formula').val();
    $.ajax({
        url: serverURL + 'getDocRequestSendDetail2?docno=' + docno + '&p=' + formula + '&wh=' + from_wh,
        method: 'GET',
        cache: false,
        success: function (res) {
            console.log(res);
...
function _displayTable() {
    ...
    for (var i = 0; i < item_detail.length; i++) {
        ...
        html += '<td class="text-center" ><input type="number" style="text-align:center" class="qty_edit  qty_value_' + i + '" data-index="' + i + '" value="' + item_detail[i].event_qty + '"></td>'

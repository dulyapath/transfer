...
$(document).ready(function () {
    ...
    $(document).delegate('.show_refund', 'click', function (event) {
        var code = $(this).attr('data-docno')
        var fg = $(this).attr('data-fg')

        swal({
            title: "ยืนยันการทำงาน",
            text: "ต้องการยกเลิกใบรับ " + code + " ใช่หรือไม่",
            icon: "warning",
            buttons: ["ปิด", "ตกลง"],
            dangerMode: true,
        })
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url: serverURL + 'cancelDoc',
                            method: 'POST',
                            data: {doc_no: code, fg: fg},
                            success: function (res) {
                                swal("การทำรายการทำเสร็จ", {
                                    icon: "success",
                                });
    ...
    $(document).delegate('.show_detail', 'click', function (event) {
        ...
        window.location.href = "formdetail.jsp?d=" + code + "&s=" + status
...
function _getListData(data) {
    ...
    $.ajax({
        url: serverURL + 'getRefunddoc?fd=' + from_date + '&td=' + to_date + '&search=' + data,
        method: 'GET',
        cache: false,
        success: function (res) {
...
function _showListDetail(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (data[i].status == '4') {
            ...
            html += `   <div class="col-4">
                        <div class="text-body text-left">เลขที่ใบรับเข้า : ${data[i].fg_doc}</div>
                    </div>`
            html += `   <div class="col-4">
                        <div class="text-body text-left">เลขที่ใบรับคืน : ${data[i].rim_doc}</div>
                    </div>  </div>`
        }
        ...
        if (data[i].status == "2") {
            html += `<button class="btn btn-success btn-icon-split send_approve" data-status="${data[i].status}" data-docno="${data[i].doc_no}">
                            ...
                            <span class="text">รับสินค้า</span>
                        </button>`
            ...
            html += `<button class="btn btn-danger btn-icon-split show_refund" data-docno="${data[i].doc_no}" data-fg="${data[i].fg_doc}" data-status="${data[i].status}" style='margin-right:5px;'>
                            <span class="icon text-white-50">
                                <i class="fas fa-redo-alt"></i>
                            </span>
                            <span class="text">ยกเลิกใบรับ</span>
                        </button>`

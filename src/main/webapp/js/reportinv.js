...
function _Process() {
    ...
    item_detail = [];
    $.ajax({
        url: serverURL + 'getReportInv?fromdate=' + from_date + '&todate=' + to_date + '&custcode=' + ar_code + '&salecode=' + sale_code,
        method: 'GET',
        cache: false,
        success: function (res) {
            console.log(res)
...
function _showDetail() {
    ...
    if (item_detail.length > 0) {
        ...
        html += `<th class="text-center">วันที่</th>
                                    <th nowrap class="text-center">เลขที่เอกสาร</th>
                                    <th nowrap class="text-center">ชื่อลูกค้า</th>
                                    <th nowrap class="text-center">ยอดขาย</th>
                                    <th nowrap class="text-center">กำไร</th>
                                    <th nowrap class="text-center">% กำไร</th>
                                    <th nowrap class="text-center">พนักงานขาย</th>
                                    <th nowrap class="text-center">พนักงาน Admin</th>
                                </tr>`
        ...
        item_detail.forEach(function (data, index) {
            total_sale += data.total_value;
            total_profit += data.total_profit;
            total_profit_percent += data.total_profit_percent;
            console.log()
            if (data.sale_code == undefined) {
                data.sale_code = '';
            }
            if (index % 2) {
            ...
            html += `               <td nowrap>${formatdate(data.doc_date)}</td>
                                    <td nowrap>${data.doc_no}</td>
                                    <td nowrap>${data.cust_code + '~' + data.cust_name}</td>
                                    <td class="text-right" nowrap>${formatnumber(data.total_value)}</td>
                                    ...
                                    <td nowrap>${data.sale_code + '~' + data.sale_name}</td>
                                    <td nowrap>${data.creator_code + '~' + data.creator_name}</td>
                                </tr>`
            if (showDetail) {
                html += `       <tr style='background-color:#fff9c4'>
                                    <td class="text-center">สินค้า</td>
                                    <td class="text-center">จำนวน</td>
                                    <td class="text-center">ราคาขาย</td>
                                    <td class="text-center">ราคาทุน</td>
                                    <td class="text-center">รวมขาย</td>
                                    <td class="text-center">รวมทุน</td>
                                    <td class="text-center">กำไร</td>
                                    <td class="text-center">%กำไร</td>
                                </tr>`
            }
            if (showDetail) {
                ...
                data.detail.forEach(function (detail, i) {
                    ...
                    html += `       <tr>
                                    <td nowrap>${detail.item_code + '~' + detail.item_name + ' ' + detail.unit_name + '(' + detail.unit_code + ')'}</td>
                                    <td class="text-right">${formatnumber2(detail.qty)}</td>
                                    <td class="text-right">${formatnumber(detail.price)}</td>
                                    <td class="text-right">${formatnumber(detail.sum_amount)}</td>
                                    <td class="text-right">${formatnumber(detail.lao_cost)}</td>
                                    <td class="text-right">${formatnumber(detail.sum_cost)}</td>
                                    <td class="text-right">${formatnumber(detail.profit)}</td>
                                    <td class="text-right">${formatnumber(detail.profit_percent)}%</td>
                                </tr>`
                });
        ...
        html += `<tr  style='background-color:#e3f2fd'>
                                    ...
                                    <td ></td>
                                    <td ></td>
                                    <td class="text-right">${formatnumber(total_sale)}</td>
                                    <td class="text-right">${formatnumber(total_profit)}</td>
                                    <td class="text-right">${formatnumber(total_profit_percent)}%</td>
                                    <td ></td>
                                    <td ></td>
                                </tr>`
        html += `</tbody>`
